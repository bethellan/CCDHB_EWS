...
function playGentleAlarmPulse(){
  try{
    ...
    master.gain.exponentialRampToValueAtTime(0.08,now+0.035);
    ...
    [0,0.22].forEach((offset,i)=>{
      ...
      osc.type='sine';
      osc.frequency.setValueAtTime(i===0?660:520,now+offset);
      toneGain.gain.setValueAtTime(0.0001,now+offset);
      toneGain.gain.exponentialRampToValueAtTime(0.85,now+offset+0.025);
      toneGain.gain.exponentialRampToValueAtTime(0.0001,now+offset+0.18);
...
function startEmergencyAlarm(){
  ...
  alarmTimer=setInterval(playGentleAlarmPulse,4200);
}
