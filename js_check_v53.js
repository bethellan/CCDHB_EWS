...
let audioCtx=null,alarmTimer=null,alarmSilenced=false,alarmPrimed=false;
...
function playSoftTripleAlarmPulse(){
  try{
    ...
    master.gain.exponentialRampToValueAtTime(0.30,now+0.025);
    master.gain.exponentialRampToValueAtTime(0.0001,now+0.86);
    master.connect(audioCtx.destination);

    // Soft triple beep: clear enough for MET/PET, less harsh than a monitor alarm.
...
function startEmergencyAlarm(){
  ...
  alarmTimer=setInterval(playSoftTripleAlarmPulse,5200);
}
...
function openRespiratoryDistressGuide(param){
  ...
  $('respGuideBody').innerHTML=`<p class="guide-intro">Select the signs that apply. Continue will apply the highest selected severity.</p><table class="distress-guide-table"><thead>${head}</thead><tbody>${body}</tbody></table><div class="guide-note">${escapeHtml(rule)}${note?'<br>'+escapeHtml(note):''}</div><div class="guide-actions"><button id="respGuideCancelBtn" class="nav-button guide-cancel" type="button">Cancel</button><button id="respGuideContinueBtn" class="nav-button guide-continue" type="button">Continue</button></div>`;
...
function applyRespiratoryDistressGuide(){
  ...
  if(!severities.length){closeRespiratoryDistressGuide();return;}
  const selectedSeverity=severities.includes('severe')?'severe':(severities.includes('moderate')?'moderate':'mild');
...
function renderSummary(){
  ...
  $('summaryResult').className='result-box '+(lastSummary.emergency?'met-alert emergency-mega':'');
...
async function init(){config=await loadConfig();document.addEventListener('pointerdown',primeAlarmAudio,{once:true,passive:true});bindTap($('adultBtn'),startAdult);bindTap($('paedBtn'),startPaediatricAgeMenu);attachLongPressBack($('ageBackBtn'),goMainMenu,goMainMenu);attachLongPressBack($('entryBackBtn'),goEntryBack,goMainMenu);bindTap($('helpBtn'),renderHelp);bindTap($('silenceAlarmBtn'),()=>{stopEmergencyAlarm(true);$('silenceAlarmBtn').classList.add('hidden');});bindTap($('closeHelpBtn'),closeHelp);bindTap($('closeRespGuideBtn'),closeRespiratoryDistressGuide);$('helpModal').addEventListener('click',e=>{if(e.target===$('helpModal'))closeHelp();});$('respDistressGuideModal').addEventListener('click',e=>{if(e.target===$('respDistressGuideModal'))closeRespiratoryDistressGuide();});blockZoomGestures();blockTextSelectionAndCallout();showScreen('mainMenu');}
