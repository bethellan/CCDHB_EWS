...
function playGentleAlarmPulse(){
  try{
    ...
    master.gain.exponentialRampToValueAtTime(0.20,now+0.035);
    master.gain.exponentialRampToValueAtTime(0.0001,now+1.05);
    master.connect(audioCtx.destination);

    // Selected gentle chime: soft chord with a short shimmer, less harsh than a beep.
    const tones=[
      {freq:523.25,start:0.00,duration:0.82,gain:0.58,type:'sine'},
      {freq:783.99,start:0.00,duration:0.92,gain:0.42,type:'sine'},
      {freq:1046.50,start:0.045,duration:0.58,gain:0.18,type:'triangle'}
    ];
    tones.forEach(t=>{
      ...
      gain.gain.exponentialRampToValueAtTime(t.gain,start+0.045);
...
function startEmergencyAlarm(){
  ...
  playGentleAlarmPulse();
  alarmTimer=setInterval(playGentleAlarmPulse,5200);
}
...
async function init(){config=await loadConfig();document.addEventListener('pointerdown',primeAlarmAudio,{once:true,passive:true});$('adultBtn').addEventListener('click',startAdult);$('paedBtn').addEventListener('click',startPaediatricAgeMenu);attachLongPressBack($('ageBackBtn'),goMainMenu,goMainMenu);attachLongPressBack($('entryBackBtn'),goEntryBack,goMainMenu);$('helpBtn').addEventListener('click',renderHelp);$('silenceAlarmBtn').addEventListener('click',()=>{stopEmergencyAlarm(true);$('silenceAlarmBtn').classList.add('hidden');});$('closeHelpBtn').addEventListener('click',closeHelp);$('helpModal').addEventListener('click',e=>{if(e.target===$('helpModal'))closeHelp();});blockZoomGestures();blockTextSelectionAndCallout();showScreen('mainMenu');}
