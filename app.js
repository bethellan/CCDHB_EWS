...
Re-calculate full PEWS after interventions.
Document plan, including timeframe, criteria for review and frequency of vital signs.
Communicate plan to whānau."},{"id":"pews-8-plus","minScore":8,"maxScore":null,"label":"PEWS 8+","colour":"red","actionTitle":"PEWS 8+","actionText":"Notify ACNM/Nurse in Charge.
...
function selectedValueLabel(entry){const p=entry.parameter;if(entry.choice.kind==='range')return `${displayRange(entry.choice.item,p)}${p.unit?' '+p.unit:''}`;return `${entry.choice.item.label}`;}
